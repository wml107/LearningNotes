是前人对开发经验的总结，用来提高代码复用性、维护性、可读性、稳健性、安全性，以及提升开发效率的一系列软件开发方案。这些思想在几乎所有的计算机软硬件设计领域，都是通用的。
...
这六大原则通篇看完，我的感受就是主要在做两件事：1.尽可能减少未来可能出现的变更导致的扩散；2.让程序的结构更加清晰。
...
最少知道原则，尽量降低类与类之间的耦合，一个对象应该对其他对象有最少的了解。相比于另外几种原则，这条更倾向于是对程序内部数据交互的限制。

例如：类A调用了类B的某个方法，该方法需要A中的某个对象的某些字段作为参数，那么这时我们就应当在A中的调用处，将对象的字段传入方法，而不是直接传入对象，有方法再去获取这个对象字段。因为这个对象一旦发生变动，这样做就意味着你要在这些需要该对象的方法处都做出修改，要修改的地方会变得到处都是。
...
两个具体的例子：你设计一个程序，对于有着不同需求的使用者，你应当通过一个配置文件，来实现这些差异，让他们按需进行配置，而不是直接改动你的代码；使用抽象类。
...
不过现在的开发种，设计阶段这东西已经用的很少了。这里只是提一嘴，以及后面设计模式的描述种可能会用到这种图，简单了解即可。
